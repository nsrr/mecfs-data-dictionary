...
# Check if De-ID # are the same as original EDF filename
inexact_matches_df <- id_link_data[!is.na(id_link_data$edf_filename), ] %>%
  mutate(original_edf_basename = tools::file_path_sans_ext(edf_filename),
         clean_edf_basename = gsub("_deidentified$", "", original_edf_basename),
         exact_match = ifelse(`De-ID #` == clean_edf_basename, "Yes", "No")) %>%
  select(MapID, `MECFS ID`, `De-ID #`, clean_edf_basename, exact_match) %>%
  rename(DeID = `De-ID #`, 
         MECFS_ID = `MECFS ID`,
         Clean_EDF_Basename = clean_edf_basename,
